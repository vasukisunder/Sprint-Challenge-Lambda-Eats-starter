...
React's 'useRouteMatch" allows you to change a specific value in a url and have different UIs display without having to create entire new routes for each one. With just one route, you can use ${url} and ${path} to replace redundant links and routes.

2. How would you explain form validation to someone who has never programmed before?

Form validation occurs when you fill out a form and do not enter values that are considered "valid," such as an email that isn't formatted as an email, or a password that is not strong enough.

3. In 1-2 sentences, define end to end testing. 

The entire user experience is tested in end to end testing, by imitating the actions that an actual user may take when interacting with the application. It ensures that the actions you have coded are actually achievable by a user.
